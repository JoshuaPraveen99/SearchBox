...
function getDropdownLimit(dropdownId) {
  return DROPDOWN_LIMITS[dropdownId] || 50; // Default 50 if not specified
}
...
var ABSOLUTE_MAX_CHIPS = 50; // For backward compatibility

var _batchMode = false;
var _batchLabelsAdded = [];
...
function selectAllNotifications(e){
  ...
  for (var j=0; j<labelsToAdd.length; j++){
    ...
    if (after > before) {
      successCount++;
      _batchLabelsAdded.push(labelsToAdd[j]);
    }
  }
  
  _batchMode = false;
  
  if (typeof toggleNotif==="function" && _batchLabelsAdded.length > 0) {
    for (var k=0; k<_batchLabelsAdded.length; k++){
      try {
        toggleNotif(_batchLabelsAdded[k], true);
      } catch(err) {
...
function clearAllNotifications(e){
  ...
  if (typeof toggleNotif==="function") {
    for (var k=0; k<labelsToRemove.length; k++){
      try {
        toggleNotif(labelsToRemove[k], false);
      } catch(err) {
        console.error('[Dropdown] toggleNotif error:', err);
      }
    }
...
function addChip(lbl){
  ...
  if (!_batchMode && typeof toggleNotif==="function") {
    try {
...
/* ✅ ENHANCED Select All with Limits for Each Dropdown */
  ...
  for (var i=0;i<dds.length;i++){
    (function(drop){
      ...
      for (var k=0;k<boxes.length;k++){
        ...
        // ✅ ENHANCED: Add limit check for individual checkbox changes
        (function(checkbox, d, b, l, limit, name){
          checkbox.addEventListener("change", function(e){ 
            if (this.checked) {
              // Check limit before allowing check
              var currentCount = 0;
